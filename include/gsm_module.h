...
public:
    ...
    // Initialize GSM module on UART2 with default SIM800L baud rate (9600)
    void begin(unsigned long baudRate = 9600) {
        gsmSerial.begin(baudRate, SERIAL_8N1, 16, 17); // RX2=16, TX2=17 (default UART2)
        delay(1000);
...
private:
    bool ATCommandOK(String cmd, bool extendTime) {
        ...
        unsigned long t = millis();
        String str;
        unsigned long timeout = extendTime ? 6000L : 4000L;

        while (millis() - t < timeout) {
            if (gsmSerial.available() > 0) {
                str = readString();
                if (str.indexOf("OK") != -1) return true;
                else if (str.indexOf("ERROR") != -1) return false;
            }
        }
        Serial.println("[GSM] Command timeout");
        return false;
