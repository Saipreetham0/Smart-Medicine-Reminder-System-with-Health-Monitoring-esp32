...
//   // // Initialize the GSM module with proper error checking
//   // Serial.println("Initializing GSM module...");
//   // if (gsm.begin(9600))
//   // { // Be explicit about the baud rate
//   //   Serial.println("GSM module initialized successfully!");
//   // }
//   // else
//   // {
//   //   Serial.println("WARNING: GSM module initialization failed! SMS and calling features will not work.");
//   // }
...
//   // Initialize GSM module with proper error checking
//   Serial.println("Initializing GSM module...");
//   for (int attempt = 1; attempt <= 3; attempt++)
//   {
//     Serial.print("Attempt ");
//     Serial.print(attempt);
//     Serial.println(" to initialize GSM module...");
...
//     else
//     {
//       Serial.println("WARNING: All GSM initialization attempts failed! SMS and calling features will not work.");
//     }
//   }
...
// GSM Module setup using Hardware Serial2
GSMModule gsm(Serial2);
...
void setup() {
  ...
  // Initialize GSM module
  gsm.begin();  // Initialize with default baud rate (9600)

  // Test GSM module with signal quality check
  Serial.println("Testing GSM module connection...");
  if (gsm.ATCommandOK("AT+CSQ", true)) {
    Serial.println("GSM module connected and responding");
  } else {
    Serial.println("Warning: GSM module not responding properly");
  }
  ...
  // Send a test SMS to verify GSM module is working
  if (g_patientPhone.length() > 0) {
    gsm.sendSMS(g_patientPhone, "Smart Medicine Reminder system is now online.");
    Serial.println("Sent startup notification SMS");
  }
}
    ...
    for (int i = 0; i < g_numSchedules; i++) {
      if (g_schedules[i].alarmActive && !g_schedules[i].taken) {
        ...
        if (chamberIndex >= 0 && chamberIndex < NUM_CHAMBERS) {
          ...
          // Send confirmation SMS if phone number is set
          if (g_patientPhone.length() > 0) {
            String message = "Medicine from Chamber ";
            message += String(chamberIndex + 1);
            message += " has been taken at ";
            message += g_currentTimeString;

            gsm.sendSMS(g_patientPhone, message);
            Serial.println("Sent medicine taken confirmation SMS");
          }
        }
      }
    }
...
void checkSchedules() {
  ...
  for (int i = 0; i < g_numSchedules; i++) {
    ...
    if (g_schedules[i].hour == g_currentHour &&
      ...
      if (g_patientPhone.length() > 0) {
        String reminderMsg = "REMINDER: Time to take medicine from Chamber ";
        reminderMsg += String(g_schedules[i].chamberIndex + 1);
        // gsm.sendSMS(g_patientPhone, reminderMsg);
        Serial.println("Sent initial medication reminder SMS");
      }
    }
    ...
    if (g_schedules[i].alarmActive && !g_schedules[i].taken) {
      ...
      if (!g_schedules[i].callSent &&
        ...
        // If patient phone is set, make the calls
        if (g_patientPhone.length() > 0) {
          Serial.print("[");
          Serial.print(g_currentTimeString);
          Serial.print("] Making 3 reminder calls to ");
          Serial.println(g_patientPhone);

          // Make 3 calls with 30 second delay between them
          gsm.makeMultipleCalls(g_patientPhone, 3, 30000);

          // Also send an SMS reminder
          String message = "URGENT: Please take your medicine from Chamber " +
                           String(g_schedules[i].chamberIndex + 1) +
                           " scheduled for " +
                           String(g_schedules[i].hour) + ":" +
                           String(g_schedules[i].minute);
